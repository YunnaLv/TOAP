...
def get_args_parser():
    ...
    parser.add_argument('--gpu_id', default="2", type=str, help="""Define GPU id.""")
    parser.add_argument('--data_dir', default="./data/", type=str, help="""Path to dataset.""")
    parser.add_argument('--dataset', default="CASIA", type=str, help="""Dataset name: imagenet, nuswide_m, coco.""")
    ...
    parser.add_argument('--encoder', default="ResNet34", type=str,
                        help="""Encoder network: ResNet, AlexNet, ViT, DeiT, SwinT.""")
    ...
    parser.add_argument('--lambda1', default=0.1, type=float,
                        help="""Balancing hyper-paramter on self knowledge distillation.""")
    ...
    parser.add_argument('--transformation_scale', default=0.5, type=float,
                        help="""Transformation scaling for self teacher: AlexNet=0.2, else=0.5.""")
...
def train(args):
    ...
    if dname == 'CASIA' or dname == 'vggfaces2':
        NB_CLS = 28
        Top_N = 300
    else:
    ...
    Norm = nn.Sequential(Kg.Normalize(mean=T.as_tensor(mean_), std=T.as_tensor(std_)))
    ...
    if args.encoder == 'AlexNet':
        ...
        Baseline = ResNet(args.encoder)
        fc_dim = 2048
        if args.encoder == 'ResNet34':
            fc_dim = 512
    elif 'Vgg' in args.encoder:
        Baseline = Vgg(args.encoder)
        fc_dim = 4096
    else:
        print("Wrong dataset name.")
        return
