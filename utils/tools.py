...
def config_dataset(config):
    ...
    if config["dataset"] == "imagenet-50":
        config["topK"] = 100
        config["n_class"] = 50
        config["data_path"] = "/data/UTAH_datasets/imagenet-50"
    elif config["dataset"] == "vggfaces2":
        config["topK"] = 300
        config["n_class"] = 28
        config["data_path"] = "/data/UTAH_datasets/vggfaces2/"
        ...
        config["topK"] = 300
        config["n_class"] = 28
        config["data_path"] = "/data/UTAH_datasets/CASIA-WebFace/"    # 因为database.txt文件里存的是绝对路径
...
def image_transform(resize_size, crop_size, data, dataset):   # data(train, test, database), dataset(CASIA..)
    ...
    return transforms.Compose([transforms.Resize((resize_size, resize_size))]  # 所作变换
                              + step +
                              [transforms.ToTensor(),
...
class ImageList_(object):

    def __init__(self, data_path, image_list, target, transform):
        # imgs = [(path, label)]
        self.imgs = [(data_path + val.split()[0], np.array([int(la) for la in val.split()[1:]])) for val in image_list]
        self.target = target
        self.transform = transform  # 图像变换
    ...
    def __getitem__(self, index):
        path, label = self.imgs[index]
        tar = self.target[index]
        img = Image.open(path).convert('RGB')   # Image.open打开图像 path -> jpeg
        img = self.transform(img)
        return img, label, index, tar
